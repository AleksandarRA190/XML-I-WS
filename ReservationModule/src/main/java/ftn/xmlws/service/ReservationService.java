...
public class ReservationService {
	...
	public boolean makeReservation(ReservationDTO reservation) {
		...
		// r.setGuest(restTemplate.getForObject("http://localhost:9006/users/user/"+reservation.getGuest().getUsername(),
		// ftn.xmlws.model.User.class));// ovde poziv mikroservisu za uzer
		r.setConfirmed(false);
		r.setDeleted(false);
		r.setFromDateTime(reservation.getFromDateTime());
		r.setToDateTime(reservation.getToDateTime());

		List<PeriodPrice> periodPrices = r.getAccommodationUnit().getPeriodPrices();
		double price = 0;

		if (periodPrices.size() > 0) {
			for (PeriodPrice p : periodPrices) {
				if (r.getFromDateTime().toLocalDate().isAfter(p.getFromDate())
						&& r.getFromDateTime().toLocalDate().isBefore(p.getToDate())) {
					price = 0;
					for (LocalDate date = r.getFromDateTime().toLocalDate(); date
							.isBefore(r.getToDateTime().toLocalDate()); date = date.plusDays(1))
						price += p.getPrice();
					break;
				}
			}
		...
		if(price == 0) {
			for (LocalDate date = r.getFromDateTime().toLocalDate(); date.isBefore(r.getToDateTime().toLocalDate()); date = date.plusDays(1)) {
				price += r.getAccommodationUnit().getDefaultPrice();
