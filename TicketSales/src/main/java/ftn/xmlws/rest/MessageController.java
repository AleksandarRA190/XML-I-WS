...
public class MessageController {
	...
	public ResponseEntity<List<MessageDTO>> getMessagesByReservation(@PathVariable("id") Long id) {
		...
		Messages retVal = restTemplate.getForObject("http://localhost:9006/message/byReservation/"+id, Messages.class);
	...
	public ResponseEntity<MessageDTO> getMessage(@PathVariable("id") Long id) {
		MessageDTO message = restTemplate.getForObject("http://localhost:9006/message/"+id, MessageDTO.class);
		if (message == null) {
	...
	public ResponseEntity<Void> sendMessage(@RequestBody MessageDTO message) {
		...
		restTemplate.put("http://localhost:9006/message/send", request);
	...
	public ResponseEntity<Void> respondMessage(@PathVariable("id") Long id, @RequestBody MessageDTO message) {
		...
		restTemplate.put("http://localhost:9006/message/respond/"+id, request);
